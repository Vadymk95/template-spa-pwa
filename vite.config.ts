...
export default defineConfig(({ command }) => ({
    ...
    plugins: [
        ...
        VitePWA({
            registerType: 'prompt',
            // `injectRegister: 'inline'` (NOT 'auto'): the app imports
            // `virtual:pwa-register/react` for the update toast (`usePwaUpdateToast.ts`).
            // In 'auto' mode, vite-plugin-pwa detects that import and DISABLES
            // its registration injection — assuming the consumer drives it.
            // Registration then only happens inside `useRegisterSW()` once
            // `PwaUpdateToast` mounts, which is gated on `isI18nReady` in
            // `main.tsx`. If i18n init fails, the fallback renders and
            // `useRegisterSW` never runs → SW never registers. 'inline' injects
            // the registration `<script>` into `index.html` so SW boots before
            // React touches the DOM, regardless of i18n state. The hook still
            // tracks `needRefresh` for the toast — `register()` is idempotent.
            ...
            workbox: {
                ...
                // `bundle-analysis.html` is generated by ANALYZE=true builds via
                // rollup-plugin-visualizer; it's a 700KB dev artefact that must NEVER
                // ship into the SW precache (would bloat install + cache eviction risk).
                globIgnores: [
                    '**/mockServiceWorker.js',
                    '**/mockServiceWorker.js.br',
                    '**/bundle-analysis.html',
                    '**/bundle-analysis.html.br'
                ],
